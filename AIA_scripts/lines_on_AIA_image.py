### In this plot we will plot the radio sources and line joining them on the AIA ratio image.
# Importing modules 
...
def joinpoints(gauss_file, fint=0, tint=0,colour = 0):
	...
	### To plot the points 
	ax1.plot(xdeg*u.deg, ydeg*u.deg,'o',color=collist[colour],transform=ax1.get_transform("world"))
...
xlims_world = [17, 727]*unit.arcsec
ylims_world = [-930, -150]*unit.arcsec
...
for i in range(8,gauss_file1.shape[1]-1):
	joinpoints(gauss_file1, fint=0, tint=i,colour = i)
...
cmap = mpl.cm.Reds
norm = mpl.colors.Normalize(vmin=1, vmax=287)
