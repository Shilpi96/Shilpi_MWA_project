#### Band-split figure for the paper. It's in the folder /AIA_data/scripts
...
def overplot(gauss_file,ax1,fint=0, tint=0,colour = 0,color = 'Blues',marker = 'o', label = '133.36 MHz'):
	...
	ax1.plot(xdeg, ydeg, marker, color=collist[colour], mec = 'Black',transform=ax1.get_transform('world'), markersize=10, label = label)
	return ax1
...
def joinpoints(gauss_HF,gauss_LF,ax1,Ufint = 0, Utint = 0,Lfint = 0, Ltint = 0,colour = 0):
	cm = pylab.get_cmap('Purples')
	collist = cm(np.linspace(0, 255, 16).astype(int))   #### Creating the colour points in a step 10 from 0 to 255 considering you have 10 points to plot

	xdeg,ydeg = getarcsec(gauss_HF, fint=Ufint, tint=Utint)
	xdeg1,ydeg1 = getarcsec(gauss_LF, fint=Lfint, tint=Ltint)
	xx = [xdeg,xdeg1]*u.deg
	yy = [ydeg,ydeg1]*u.deg
	ax1.plot(xx*u.deg, yy*u.deg,color=collist[colour],transform=ax1.get_transform("world"), mec = 'black')
	return ax1
...
#### Get the points for some angle
def plot_lines( angle, length, aia_171_map, color = 'blue', label = '20 deg'):
	...
	xlims_world = [354, endx]*unit.arcsec
	ylims_world = [-348, -endy]*unit.arcsec
	...
	ax1.plot(pixel_coords[0], pixel_coords[1],color=color, linewidth = 2, label = label)
...
xlims_world = [-50, 1100]*unit.arcsec
ylims_world = [-1200, 0]*unit.arcsec
world_coords = SkyCoord(Tx=xlims_world, Ty=ylims_world, frame=aia_171_map.coordinate_frame)
pixel_coords = aia_171_map.world_to_pixel(world_coords)
...
label = ['20 deg','40 deg','60 deg','80 deg','120 deg']
color = [collist[7], collist[12], 'green', collist[17], collist[24]]
for j in range(5):
	if j<4: plot_lines( 20+j*20, 360, aia_171_map, color = color[j], label = label[j])
	if j ==4 : plot_lines( 40+j*20, 360, aia_171_map, color = color[j], label = label[j])

#ax1.legend(loc = 'upper right')
ax1.text(0.04,0.95,'AIA 193 2014-09-28 02:49:06.84',font = '7',color = 'black', transform=ax1.transAxes)
ax1.text(0.04,0.95,'AIA 193 2014-09-28 02:49:06.84',font = '7',color = 'green', transform=ax1.transAxes,bbox ={'facecolor':'white', 'alpha':0.3})
ax1.text(0.04,0.9,'AIA 171 2014-09-28 02:49:11.34',color = 'black')
ax1.text(0.04,0.9,'AIA 171 2014-09-28 02:49:11.34',color = 'red', transform=ax1.transAxes,bbox ={'facecolor':'white',
               'alpha':0.3})
ax1.text(0.04,0.85,'AIA 211 2014-09-28 02:49:11.63',color = 'blue', transform=ax1.transAxes,bbox ={'facecolor':'white',
               'alpha':0.3})
...
overplot(gauss_file0,ax1,fint = 0, tint =185,colour = 3,color = 'Blues',marker = 'o', label = '133.36 MHz')  ## 02:49:11
overplot(gauss_file1,ax1,fint = 0, tint =256,colour = 5,color = 'Blues',marker = 'o', label = '120.56 MHz')  ## 02:50:23
overplot(gauss_file2,ax1,fint = 0, tint =28,colour = 7,color = 'Blues',marker = 'o', label = '109.04 MHz')   ## 02:51:30
overplot(gauss_file3,ax1,fint = 0, tint =129,colour = 8,color = 'Blues',marker = 'o', label = '98.80 MHz')  ## 02:53:11
...
overplot(Gauss_file0,ax1,fint = 0, tint =185,colour = 3,color = 'Greens',marker = 'v', label = '109.04 MHz')  ## 02:49:11
overplot(Gauss_file1,ax1,fint = 0, tint =257,colour = 5,color = 'Greens',marker = 'v', label = '98.80 MHz')  ## 02:50:23
overplot(Gauss_file2,ax1,fint = 0, tint =32,colour = 7,color = 'Greens',marker = 'v', label = '89.84 MHz')   ## 02:51:30
overplot(Gauss_file2,ax1,fint = 7, tint =129,colour = 9,color = 'Greens',marker = 'v', label = '88.06 MHz')  ## 02:53:11

###### Join the MWA points with the lines
joinpoints(gauss_file0,Gauss_file0,ax1,Ufint = 0, Utint = 185,Lfint = 0, Ltint = 185,colour = 2)  ## 02:49:11
joinpoints(gauss_file1,Gauss_file1,ax1,Ufint = 0, Utint = 256,Lfint = 0, Ltint = 257,colour = 7)  ## 02:50:23
joinpoints(gauss_file2,Gauss_file2,ax1,Ufint = 0, Utint = 28,Lfint = 0, Ltint = 32,colour = 12)  ## 02:51:30
joinpoints(gauss_file3,Gauss_file2,ax1,Ufint = 0, Utint = 129,Lfint = 7, Ltint = 129,colour = 15)  ## 02:53:11
...
plot_MWAspectra(spectra,freq,ax,ylabels,TIME,vmin,vmax,t0,t1 ,cm,cm1,Utpoint=tpoint[2], Ufpoint=fpoint[2],Ltpoint=tpoint[0],Lfpoint=fpoint[2],i = 2,colour= 7,colour1 =3)
...
for i in range(len(freq)):
	...
	ax2.add_patch(rect) 
'''
##### Fit the curve on the bandsplitting
...
##### Join these points on the learmonth spectra
cm = pylab.get_cmap('Purples')
collist = cm(np.linspace(0, 255, 16).astype(int)) 

ax2.plot([tpoint[0], tpoint[0]], [fpoint[0],fpoint[2]], color = collist[2])
ax2.plot([tpoint[1], tpoint[1]], [fpoint[1],fpoint[3]], color = collist[7])
ax2.plot([tpoint[2], tpoint[2]], [fpoint[2],fpoint[4]], color = collist[12])
ax2.plot([tpoint[3], tpoint[3]], [fpoint[3],fpoint[5]], color = collist[15])
